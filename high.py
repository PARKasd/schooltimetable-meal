...
async def on_message(message):
    ...
    if content.startswith("k/시간표"):
        ...
        date = content[10:18]
        classNM = content[8:9]
        url =f'https://open.neis.go.kr/hub/hisTimetable?key=(open.neis.go.kr에서 받은 인증키)&Type=json&pIndex=1&pSize=100&ATPT_OFCDC_SC_CODE=(교육청코드)&SD_SCHUL_CODE=(학교코드)&GRADE={grade}&CLASS_NM={classNM}&ALL_TI_YMD={date}'
        response = requests.get(url)
        response_json = json.loads(response.text)
        print(response_json)
        ...
        for a in classtime:
            try:
                resulta = response_json['hisTimetable'][1]['row'][a]['ITRT_CNTNT']
